...
wandb.init(project="EndoSfML-Testing")

_DEPTH_COLORMAP = plt.get_cmap('plasma', 256)  # for plotting

################### Options ######################
parser = argparse.ArgumentParser(description="NYUv2 Depth options")
parser.add_argument("--dataset", required=True, help="kitti or nyu", choices=['nyu', 'kitti','hamlyn'], type=str)
...
class DepthEvalEigen():
    ...
    def main(self):
        ...
        if args.dataset == 'nyu':
            ...
            #gt_depths = []
            #for gt_f in sorted(Path(args.gt_depth).files("*.npz")):
            #gt_depths = np.load(args.gt_depth)
            gt_depths = np.load(args.gt_depth, fix_imports=True, encoding='latin1')["data"]
    ...
    def evaluate_depth(self, gt_depths, pred_depths, eval_mono=True):
        ...
        for i in tqdm(range(pred_depths.shape[0])):
            ...
            if pred_depths[i].mean() != -1:
                ...
                #print(gt_depth)
                #gt = disp = colormap(gt_depth)
                #wandb.log({"gt": wandb.Image(gt.transpose(1, 2, 0))},step=i)
